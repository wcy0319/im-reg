...
if isempty(base_image)
    irows = sD(1); icols = sD(2);
    disp(['extern_greenberg_motioncorrect_nxmod: Using mean of preceding frames as TARGET!']);
else
...
span = 5; % frame span for moving average, to set the targe image

% wb = waitbar(0, 'Processing via Greenberg et al. ...');
for j = 1:nframes
    if isempty(base_image)
        % --- NX, use the mean of the preceding 3 frames as base_image.
        if j < floor(span/2)+1
            base_image = mean(data(:,:,1:span-1),3);
        elseif j > size(data,3)-floor(span/2)
            base_image = mean(data(:,:,end-span+1:end),3);
        else
            base_image = mean(data(:,:,j-floor(span/2):j+floor(span/2)),3);
        end
    end
    %calculate gradients
    xgrad = diff(base_image,1,2);    ygrad = diff(base_image,1,1);
